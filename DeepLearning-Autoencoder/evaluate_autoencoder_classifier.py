...
def evaluate_autoencoder(dataset, model):
    ...
    with torch.no_grad():
        for c in range(num_classes):
            ...
            print("accuracy is for class " , (correct/total) * 100)
        # with torch.no_grad():
        #     for data in X: #this is equivalent to val_loader
        #         image = data
        #         label = c
        #         output = model(image)
        ...
        #print('For class ', c, ' Accuracy of the network on the validation images: %d %%' % (100 * correct / total))
