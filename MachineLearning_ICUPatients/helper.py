...
def get_challenge_data():
    ...
    features = Parallel(n_jobs=16)(delayed(project1.generate_challenge_feature_vector)(df) for _, df in tqdm(raw_data.items(), desc='Generating feature vectors'))
    df_features = pd.DataFrame(features)
    feature_names = df_features.columns.tolist()
    X, y = df_features.values, df_labels['30-day_mortality'].values
    X = project1.impute_challenge_missing_values(X)
    X = project1.normalize_feature_matrix(X)
    np.save('feature_vector.npy', X)
    X = np.load('feature_vector.npy')
...
def generate_challenge_labels(y_label, y_score, uniqname):
    """
    Takes in `y_label` and `y_score`, which are two list-like objects that contain 
    both the binary predictions and raw scores from your SVM classifier.
