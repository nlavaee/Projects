...
private:
	...
	static Node *copy_nodes_impl(Node *node) {
		//creating new node ptr for new copied struct
		Node *copy_node;
	...
	static Node * find_impl(Node *node, const T &query, Compare less) {
		...
		//if at root and the query is equal to root
	}
	...
	static Node * insert_impl(Node *node, const T &item, Compare less) {
		...
		//if the node is an empty tree, we create a new node to represent
		//a new tree
		if (node == nullptr) {
			Node *new_node = new Node(item, nullptr, nullptr);
			return new_node;
		}
		//if the item is less than the current node, insert it to the left 
		else if (less(item, node->datum)) {
			node->left = insert_impl(node->left, item, less);
			return node;
		}
		//if the item is greater than the current node, insert it to the right 
		else {
	...
	static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
		...
		//Base Case 1: if tree is empty, return nullptr
		if (empty_impl(node)) {
			return nullptr;
		}

		//Base Case 2: if value is equal to datum and the right branch is empty,
		//return nullptr
		if ((!less(val, node->datum) && !less(node->datum, val)) || less(node->datum, val)) {
		...
		//Recursive Case 1: if the value isn't less than the root, check out the right branch
		if (!less(val, node->datum)) {
			//find the value that is the mininum in the right branch and that val
			//is less than and return the min value
			if (less(val, min_element_impl(node->right)->datum)) {
				return min_element_impl(node->right);
			}
			//if there isn't a value found, return nullptr
			else if (!less(val, max_element_impl(node->right)->datum)) {
				return nullptr;
			}
			//recurse
			else {
		...
		//Recursive Case 2: if the value is less than the root, check out the left branch
		else if (less(val, node->datum)) {
			//if the node on the left is nullptr or the value isn't less than the max element, 
			//return the root
			if ((node->left == nullptr) || !less(val, max_element_impl(node->left)->datum) ) {
				return node;
			}
			//find the value that is the mininum in the left branch and that val
			//is less than and return the min value
			else if (less(val, min_element_impl(node->left)->datum)) {
				return min_element_impl(node->left);
			}
			else {
				//recurse
