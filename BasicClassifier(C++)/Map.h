...
public:
    ...
    Iterator find(const Key_type& k) const {
        //create a pair type dummy that constructs the key type with a random value
        //(b/c value doesn't matter since we are only keeping
        //track of the element using the key type)
    ...
    Value_type& operator[](const Key_type& k) {
        //gotten from the website given above, the squigly brackets create the key with the specific key type and value to insert
    ...
    std::pair<Iterator, bool> insert(const Pair_type &val) {
        ...
        if (bst.find(val) == bst.end()) {
            bst.insert(val);
            return std::make_pair(bst.find(val), true);
        }
        //element is in the tree
        else {
            return std::make_pair(bst.find(val), false);
        }
    }
...
private:
    //need to create a BST with the right pair-type & the correct pair-comp
